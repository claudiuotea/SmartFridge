...
export const FridgeItem: React.FC<FridgeItem> = ({ match, history }) => {
  ...
  function addClicked(item: FridgeItemInterface) {
    ...
    addToFridge?.({ userId: "", type: item.type, quantity: newQuantity }, jwt);
  }
  function substractClicked(item: FridgeItemInterface) {
    ...
    let newQuantity: string = (item.quantity - 1).toString();
    addToFridge?.(
      { userId: "", type: item.type, quantity: newQuantity },
      jwt
    );
  }
  useEffect(() => {
    const item = items?.find(
      (x: FridgeItemInterface) => x.id === +match.params.id!
    );
    setItem(item);
      ...
      <IonContent>
        <IonGrid className="ion-padding ion-text-center ion-margin">
          ...
          <IonRow>
            <IonCol>
              <IonButton
                onClick={() => removeClicked(item!.id.toString())}
                color="dark"
              >
                Remove
              </IonButton>
